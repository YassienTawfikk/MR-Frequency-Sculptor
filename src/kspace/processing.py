...
def reconstruct_all_versions(kspace, prefix):
    ...
    save_reconstructed_image(f"{prefix}_full", full_img, dataset_dir)
    ...
    save_reconstructed_image(f"{prefix}_partial", partial_img, dataset_dir)
    ...
    save_reconstructed_image(f"{prefix}_lowpass", lowpass_img, dataset_dir)
    ...
    save_reconstructed_image(f"{prefix}_highpass", highpass_img, dataset_dir)

    # Save raw arrays into an npz for precise analysis later
    save_reconstructions(prefix, full_raw, partial_raw, lowpass_raw, highpass_raw, dataset_dir)

    print(f"\n{prefix}_full serves as the gold standard for comparison.")
